...
-   Includes boxes for Theorems, Propositions, Corollaries and Lemmas (with proofs), and Definitions and Examples - defined in [`boxes.sty`](/latex-lecture-notes/packages/boxes.sty)
-   Handles header imports, document setup, and shortcuts (like `\Q` for $\mathbb{Q}$ and `\R` for $\mathbb{R}$ etc. - defined in [`general.sty`](/latex-lecture-notes/packages/general.sty)
-   Also in the repository is [`Lecture Notes Template.tex`](/latex-lecture-notes/Lecture%20Notes%20Template.tex) containing use cases for all the environments, and some of the shortcuts.
